...
namespace SuperBit {
    ...
    export function RGB_Program(): neopixel.Strip {
        if (!LED_Strip) {
            LED_Strip = neopixel.create(DigitalPin.P12, 4, NeoPixelMode.RGB);
        }
        return LED_Strip;
    }
    ...
    //% blockId=SuperBit_Motor block="Motor |Motor1 speed %speed1|Motor2 speed %speed2|Motor3 speed %speed3|Motor4 speed %speed4|"
    //% weight=92
    //% blockGap=50
    //% speed1.min=-255 speed1.max=255
    //% speed2.min=-255 speed2.max=255
    //% speed3.min=-255 speed2.max=255
    //% speed4.min=-255 speed2.max=255
    //% name.fieldEditor="gridpicker" name.fieldOptions.columns=2
    export function MotorRunDual(speed1: number, speed2: number, speed3: number, speed4: number): void {
